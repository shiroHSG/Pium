...
public class JwtFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
        ...
        return path.startsWith("/ws/"); // WebSocket 제외
    }
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        boolean isAllowed = ALLOWED_URLS.stream().anyMatch(pattern -> pathMatcher.match(pattern, uri));
        ...
        if (header != null && header.startsWith("Bearer ")) {
            ...
            if (token == null || token.equals("null") || token.trim().isEmpty()) {
                ...
                sendUnauthorized(response, "Token is null or empty");
                return;
            }
            ...
            try {
                ...
                if (tokenService.isAccessTokenBlacklisted(token)) {
                    System.out.println("[JwtFilter] 블랙리스트 토큰 → 401 응답");
