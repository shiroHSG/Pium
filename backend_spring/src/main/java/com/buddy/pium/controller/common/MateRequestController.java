...
public class MateRequestController {
    ...
    public ResponseEntity<Map<String, String>> requestMate(@PathVariable Long receiverId,
        ...
        mateRequestService.requestMate(senderId, receiverId);
    ...
    public ResponseEntity<Map<String, String>> acceptMateBySender(@PathVariable Long senderId,
        ...
        mateRequestService.acceptMateBySender(senderId, receiverId);
    ...
    public ResponseEntity<Map<String, String>> rejectMateBySender(@PathVariable Long senderId,
                                                                  @CurrentMemberId Long receiverId) {
        mateRequestService.rejectMateBySender(senderId, receiverId);
        return messageResponse("Mate 요청을 거절했습니다.");
    }

    // Mate 연결 해제
    @PostMapping("/disconnect")
    public ResponseEntity<Map<String, String>> disconnectMate(@CurrentMemberId Long memberId) {
        mateRequestService.disconnectMate(memberId);
        return messageResponse("Mate 연결이 해제되었습니다.");
    }

    // 받은 Mate 요청 목록 조회
    @GetMapping("/received")
    public ResponseEntity<List<MateResponseDto>> getReceivedRequests(@CurrentMemberId Long receiverId) {
        return ResponseEntity.ok(mateRequestService.getPendingRequests(receiverId));
    }

    // 보낸 Mate 요청 목록 조회
    @GetMapping("/sent")
    public ResponseEntity<List<MateResponseDto>> getSentRequests(@CurrentMemberId Long senderId) {
        return ResponseEntity.ok(mateRequestService.getSentRequests(senderId));
    }
    ...
    public ResponseEntity<Map<String, String>> cancelRequest(@PathVariable Long receiverId,
                                                             @CurrentMemberId Long senderId) {
        mateRequestService.cancelRequest(senderId, receiverId);
